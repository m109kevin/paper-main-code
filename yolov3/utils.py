...
def draw_location(image, bboxes, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence = True, Text_colors=(255,255,0), rectangle_colors='', tracking=False):   
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if show_label:
            ...
            try:
                ...
                if label_name == 'car':
                    ...
                    print('detect:',detect)
                if label_name == 'one':
                    print('location_one = ',x2)
                    return [detect,x2]
                elif label_name == 'three':
                    read_three = True
                    x_three = x2
                    continue
                elif label_name == 'two':
                    print('location_two = ',x2)
                    return [detect,x2]
        ...
        if read_three:
            print('location_three = ',x_three)
            return [detect,x_three]
...
def detect_realtime_once(Yolo, output_path, input_size=416, show=False, CLASSES=TRAIN_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    times = []
    vid = cv2.VideoCapture(0)
